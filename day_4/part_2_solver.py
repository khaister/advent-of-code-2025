...
def find_accessible_rolls(rolls: list[str]) -> tuple[int, list[tuple[int, int]]]:
    accessible_rolls = 0
    locations = []
    ...
    return accessible_rolls, locations


def remove_accessible_rolls(
    rolls: list[str], locations: list[tuple[int, int]]
) -> list[str]:
    removed_count = 0
    rolls_list = [list(row) for row in rolls]
    for row, col in locations:
        rolls_list[row][col] = EMPTY
        removed_count += 1
    return ["".join(row) for row in rolls_list], removed_count


total_removed = 0
accessible_roll_count, locations = find_accessible_rolls(rolls)
while accessible_roll_count > 0:
    print(f"Accessible rolls: {accessible_roll_count}")

    rolls, removed_count = remove_accessible_rolls(rolls, locations)
    total_removed += removed_count
    print(f"Removed {removed_count} accessible rolls")

    accessible_roll_count, locations = find_accessible_rolls(rolls)

print(f"Total accessible rolls removed: {total_removed}")